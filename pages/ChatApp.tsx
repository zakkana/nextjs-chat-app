        ...
        .on(
          ...
          (payload) => {
            ...
            if (payload.eventType === "INSERT") {
              const { created_at, id, message, avatarUrl, nickName } = payload.new;
              setMessageText((messageText) => [...messageText, { created_at, id, message, avatarUrl, nickName }]);
            }
          }
        )
        .subscribe();
      ...
      <div css={wrapper}>
        {messageText.map((item) => (
          ...
          <div key={item.id} data-my-chat={item.nickName === profileFromGithub.nickName} css={chatWrapper}>
            <div css={chatProfile}>
              <time>{dateToString(item.created_at, "MM/DD HH:mm")}</time>
